 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a few neg numders"){
    assert(sum(List(-10,-40,-1)) === -51)
  }

  test("max with neg numders"){
    assert(max(List(-4,-10,-2)) === -2)
  }

  test("sum of empty list"){
    intercept[UnsupportedOperationException](sum(List()))
  }

  test("max of empty list") {
    intercept[UnsupportedOperationException](max(List()))
  }

  test("sum with repetitive elements"){
    assert(sum(List(0,0,1,1,4,4,2,2)) === 14)
  }

  test("max with repetitive elements"){
    assert(max(List(0,0,1,1,4,4,2,2)) === 4)
  }
